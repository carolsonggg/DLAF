...
control MyIngress(inout headers hdr,
    ...
    apply {
        ...
        switch (ipv4_lpm.apply().action_run) {
            ecmp_group: {
		if (hdr.ipv4.isValid()){
                    hash(meta.flow_index1, HashAlgorithm.crc16, (bit<1>)0, {hdr.ipv4.srcAddr, hdr.ipv4.dstAddr, hdr.ipv4.protocol, hdr.tcp.srcPort, hdr.tcp.dstPort, (bit<8>)1}, (bit<16>)512);        
                    hash(meta.flow_index2, HashAlgorithm.crc16, (bit<1>)0, {hdr.ipv4.srcAddr, hdr.ipv4.dstAddr, hdr.ipv4.protocol, hdr.tcp.srcPort, hdr.tcp.dstPort, (bit<8>)2}, (bit<16>)512);        
                    hash(meta.flow_index3, HashAlgorithm.crc16, (bit<1>)0, {hdr.ipv4.srcAddr, hdr.ipv4.dstAddr, hdr.ipv4.protocol, hdr.tcp.srcPort, hdr.tcp.dstPort, (bit<8>)3}, (bit<16>)512);        
                    hash(meta.flow_index4, HashAlgorithm.crc16, (bit<1>)0, {hdr.ipv4.srcAddr, hdr.ipv4.dstAddr, hdr.ipv4.protocol, hdr.tcp.srcPort, hdr.tcp.dstPort, (bit<8>)4}, (bit<16>)512);                 
	        ...
	        flow_id_tb2.read(flow_id2, (bit<32>)meta.flow_index2);
                flow_id_tb1.read(flow_id3, (bit<32>)meta.flow_index3);
	        flow_id_tb2.read(flow_id4, (bit<32>)meta.flow_index4);
                
                last_seen1.read(last_pkt_ts1, (bit<32>)meta.flow_index1);   
                last_seen2.read(last_pkt_ts2, (bit<32>)meta.flow_index2);
                last_seen1.read(last_pkt_ts3, (bit<32>)meta.flow_index3);   
                last_seen2.read(last_pkt_ts4, (bit<32>)meta.flow_index4);
		    ...
		    flowlet_id_tb2.read(meta.flowlet_id, (bit<32>)meta.flow_index2);
                    ...
                    if (standard_metadata.ingress_global_timestamp - last_pkt_ts3 > 50000) {           
            	        random(meta.flowlet_id, (bit<16>)0, (bit<16>)65000);
		        flowlet_id_tb3.write((bit<32>)meta.flow_index3, meta.flowlet_id);
                    }
                    last_seen3.write((bit<32>)meta.flow_index3, standard_metadata.ingress_global_timestamp);
		    flowlet_id_tb3.read(meta.flowlet_id, (bit<32>)meta.flow_index3);
                } else if (flow_id4 == flow_sig) {
                    if (standard_metadata.ingress_global_timestamp - last_pkt_ts4 > 50000) {           
            	        random(meta.flowlet_id, (bit<16>)0, (bit<16>)65000);
		        flowlet_id_tb4.write((bit<32>)meta.flow_index4, meta.flowlet_id);
                    }
                    last_seen4.write((bit<32>)meta.flow_index4, standard_metadata.ingress_global_timestamp);
		    flowlet_id_tb4.read(meta.flowlet_id, (bit<32>)meta.flow_index4);
                } else {
                    if (last_pkt_ts1 <= last_pkt_ts2) {
                        if (last_pkt_ts1 <= last_pkt_ts3) {
                            if (last_pkt_ts1 <= last_pkt_ts4) {
                    ...
                    } else if (last_pkt_ts2 <= last_pkt_ts3) {
                        if (last_pkt_ts2 <= last_pkt_ts4) {
                            flow_id_tb2.write((bit<32>)meta.flow_index2, flow_sig);        
                            last_seen2.write((bit<32>)meta.flow_index2, standard_metadata.ingress_global_timestamp);
            	            random(meta.flowlet_id, (bit<16>)0, (bit<16>)65000);
		            flowlet_id_tb2.write((bit<32>)meta.flow_index2, meta.flowlet_id);
                        }
                    } else if (last_pkt_ts3 <= last_pkt_ts4) {
		        ...
		        flowlet_id_tb3.write((bit<32>)meta.flow_index3, meta.flowlet_id);
                        ...
                        last_seen4.write((bit<32>)meta.flow_index2, standard_metadata.ingress_global_timestamp);
            	        random(meta.flowlet_id, (bit<16>)0, (bit<16>)65000);
		        flowlet_id_tb4.write((bit<32>)meta.flow_index4, meta.flowlet_id);
                    }  
                }     
